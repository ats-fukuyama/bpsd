    ...
    DO NSA=1,nsa_max
       ...
       SELECT CASE(ID)
          ...
          CASE(5,6) ! ADPOST PZ-variable ions
             PZA(NSA)=func_adpost(NP_A(NS),1,rte)
             PZ2A(NSA)=func_adpost(NP_A(NS),2,rte)
             PRADE(NSA,NR)=func_adpost(NP_A(NS),3,rte)
          CASE(10,11,12) ! ionization and recombination with OPEN-ADAS
             NPZ=NINT(PZA(NSA))
             NSA1=NSA_UP(NSA)
             IF(NSA1.NE.0) THEN
                CALL ADAS_scd(NP_A(NS),NPZ+1,rne,rte,DR_inz,IERR)
                CCN(NSA,NSA ,NR)=CCN(NSA,NSA ,NR) &           ! ionization
                                -DR_inz*rne                   !   to upper
                CALL ADAS_acd(NP_A(NS),NPZ+1,rne,rte,DR_rcb,IERR)
                CCN(NSA,NSA1,NR)=CCN(NSA,NSA1,NR) &           ! recombination
                                +DR_rcb*rne                   !   from upper
             END IF
             NSA1=NSA_DN(NSA)
             IF(NSA1.NE.0) THEN
                CALL ADAS_acd(NP_A(NS),NPZ,rne,rte,DR_rcb,IERR)
                CCN(NSA,NSA ,NR)=CCN(NSA,NSA ,NR) &           ! recombination
                                -DR_rcb*rne                   !     to lower
                CALL ADAS_scd(NP_A(NS),NPZ,rne,rte,DR_inz,IERR)
                CCN(NSA,NSA1,NR)=CCN(NSA,NSA1,NR) &           ! ionization
                                +DR_inz*rne                   !     from lower
             END IF
             CALL ADAS_prb(NP_A(NS),NPZ,rne,rte,DR_prb,IERR)   ! recmb/brems pwr
             CALL ADAS_plt(NP_A(NS),NPZ,rne,rte,DR_plt,IERR)   ! line ard pwr
