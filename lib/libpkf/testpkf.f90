...
1 CONTINUE
  ...
  WRITE(6,'(A)') '## ntype: 1:PDKF 2:PDKF_A 3:PDKF2 4:PGKF 9:end'
...
121      CONTINUE
        WRITE(6,'(A)') '## n1,m,x,alpha,beta,rky'
        WRITE(6,'(A,2I4,4ES12.4)') '##', n1,m,x,alpha,beta,rky
        READ(5,*,ERR=121,END=111) n1,m,x,alpha,beta,rky
        IF(n1.EQ.0) GO TO 11
        IF(n1.EQ.1.OR.n1.EQ.2) THEN
           cf_=pdkf_a(x,alpha,beta,rky,m,n1)
           WRITE(6,'(A,3ES12.4)') 'xa,cf       =',x,cf_
        ELSE
           WRITE(6,*) 'XX n1 must be 1 or 2.'
        END IF
...
131      CONTINUE
        ...
        WRITE(6,'(A/A,2I4,5ES12.4,I6)') &
          '## INPUT : n1,m,alpha,beta,rky,xmin,xmax,nxmax:', &
          '## ', n1,m,alpha,beta,rky,xmin,xmax,nxmax
        READ(5,*,ERR=131,END=111) n1,m,alpha,beta,rky,xmin,xmax,nxmax

        IF(nxmax.LT.1) GO TO 131
        ...
        IF(n1.EQ.1.OR.n1.EQ.2) THEN
           ...
           DO nx=1,nxmax
              xa(nx)=xmin+dx*(nx-1)
              cf_=pdkf_A(xa(nx),alpha,beta,rky,m,n1)
              fa(nx,1)=REAL(cf_)
              fa(nx,2)=AIMAG(cf_)
           END DO
...
141      CONTINUE
        ...
        IF(n1.EQ.1.OR.n1.EQ.2) THEN
           ...
           DO nx=1,nxmax
              ...
              DO ny=1,nymax
                 ya(ny)=log10(alpha_min)+dy*(ny-1)
                 cf_=pdkf_A(10.D0**xa(nx),10.D0**ya(ny),beta,rky,m,n1)
                 f2d(nx,ny,1)=REAL(cf_)
                 f2d(nx,ny,2)=AIMAG(cf_)
              END DO
           END DO
